...
### Numba JIT-compiled function for core computations
@njit
def process_probabilities_and_si_numba(
    HR_tissue_prob_input,  # Initial probabilities after smoothing (float, 4D)
    dGM_dist_final_in,     # Smoothed dGM distance map (float, 3D)
    dGM_mask_map_in,       # Mask for dGM regions (float, 3D)
    PVS_map_in,            # PVS binary map (boolean, 3D)
    SI_in,                 # Signal intensities for each region (float, 1D)
    dGM_py_idx_in,         # 0-based index for dGM
    NAWM_py_idx_in,        # 0-based index for NAWM
    N_true_tuple_in,       # Tuple of ints for spatial dimensions (e.g., (240, 240, 48))
    NumRegions_in          # Total number of tissue regions (int)
):
...
### Main function to create HR SI maps
